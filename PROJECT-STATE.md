...
## 2026-09-10 표지 프레임 통일 (사용자 제보)

상세 페이지 표지(`.post-cover`)에 비율 제한이 없어, 세로 사진을 cover로 넣으면 카드는 16:9로 잘리는데 상세는 원본 세로로 늘어나는 불일치가 있었다(사용자 제보 — 커스텀 사진 넣으며 발견). 카드·상세 공통 16:9 `object-fit: cover` 크롭 + `object-position: 50% 35%`(세로 사진 얼굴 상단 우선)로 통일(커밋 `23d0cf8`). 이제 표지 원본은 어떤 비율이어도 된다 — 규약은 CLAUDE.md 이미지 운영에 기록.
