...
- [ ] **Gemini 이미지 API 사용 가능하게 만들기** — 키 발급받아 `.env` 저장 완료(2026-09-10), 생성 스크립트 `scripts/generate-image.ps1` 준비 완료. **그러나 무료 할당량이 0**으로 이미지 생성 전 모델이 429 — Google Cloud 결제 연결 필요(사용자 액션). 연결되면 허브 표지 일러스트 생성부터 시작
- [ ] 초기 콘텐츠: 품종 허브 1호 + 스포크 2~3편 — 초반은 질 우선
  - **보더콜리 허브 원고 완료(2026-09-10, draft)** — 운영자 반려견 '딱지'가 보더콜리로 확정됨. 원 출처 리서치(AKC·유전검사 자료·분양 시세) 기반, 한국 실정 축(시세·아파트·여름)과 색상 축 반영. 본문에 운영자 경험 삽입 지점 표시(5개 질문 답변 대기). 검수 후 `draft: false`
  - 다음 스포크 후보: 색상 종류(기존 예시 드래프트 교체) · 운동량/산책 · 아파트 키우기
