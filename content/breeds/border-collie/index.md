...
결론부터 하면 '공간이 아니라 시간의 문제'다. 실제로 아파트에서 보더콜리를 키우는 가정이 적지 않고, 이들은 하루 2~3회, 회당 30분 안팎의 산책과 실내 놀이로 관리하고 있다. 반대로 마당 있는 집이라도 운동과 자극을 안 주면 문제행동이 생긴다. 아파트에서 키운다면 실내에서 두뇌를 쓰는 놀이(노즈워크, 퍼즐 장난감, 명령어 학습)로 에너지를 함께 소진시켜 줘야 한다. 아파트 사육의 통념·법·층간소음 기준까지 자세한 내용은 [아파트 키우기 글](/breeds/border-collie-apartment/)에서 다룬다.
...
이중모라서 평소에는 주 2~3회, 환털기(봄·가을)에는 거의 매일 빗질이 필요하다. 속털빗(언더코트 레이크)으로 죽은 속털을 걷어주면 빠짐이 눈에 띄게 줄어든다. 목욕은 필요할 때만 — 잦은 목욕은 피부 보호막을 해친다. 주기 판단 기준과 집에서 씻기는 순서는 [강아지 목욕 주기 가이드](/guides/dog-bath-schedule/)에 정리했다. 털 구조·털갈이 시기·삭모가 위험한 이유까지 깊게 다룬 글은 [보더콜리 털 관리](/breeds/border-collie-grooming/)에 있다.
