...
- [x] **Gemini 이미지 API 사용 가능** — 2026-09-10 결제 연결 완료(무료 한도 0이어서 필수였음). `scripts/generate-image.ps1`로 생성, 보더콜리 허브 표지 일러스트 생성·검수 통과 (품종 특징·팔레트 확인). **2026-09-10 실사 스타일로 전환** — 장당 약 510원(실측), 생성 규칙(한 번에 한 장·중복 없이·모델 고정)은 CLAUDE.md 참조. 허브 표지도 같은 날 실사(검흰 보더콜리 초원 전력질주)로 교체·검수 통과
